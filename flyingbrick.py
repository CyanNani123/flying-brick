...
class Flyingbrick():
    ...
    def show_scoretable_after_death(self):
        ...
        y = 150
        self.screen.blit(self.font.render("Highscores", 1, (0,0,0)), (self.width/2-130, 100))
        for score in scoretable:
            if score == self.score:
                self.screen.blit(self.font.render(str(score), 1, (255,0,0)), (self.width/2-130, y))
            else:
                self.screen.blit(self.font.render(str(score), 1, (0,0,0)), (self.width/2-130, y))
            y+=50
        self.screen.blit(self.font.render("K to return", 1, (0,0,0)), (self.width/2-130, 500))
    ...
    def reset_after_death(self):
        ...
        self.init = bool(0)
        self.score = 'Press Space'
        self.brick = pygame.Rect(10, int(self.height/2)-40, self.brick.width , self.brick.height)
        while True:
    ...
    def if_game_initialized(self):
        ...
        if self.init:
            ...
            if not len(self.object_buffer):
                self.object_buffer.append(Hindrance(self.width+40,self.height,random.randint(100,self.height-200)))
            self.apply_gravity()
